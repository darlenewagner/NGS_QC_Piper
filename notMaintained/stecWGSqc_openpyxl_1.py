...
## Prereq: export PYTHONPATH=$PYTHONPATH:/scicomp/home/ydn3/package_dir/lib/python/lib/python3.3/site-packages/
...
logger = logging.getLogger("stecWGSqc_openpyxl_1.py")
		...
		if(sheet.cell(row=wsRow, column=1).value == 'R1'):
			...
			for ii in range(1, 12):
				...
				if(re.search('depth', str(sheet.cell(row=wsRow-1 , column=ii).value), flags=re.IGNORECASE) ):
					...
					coverage = "<tr><td><b>" + str(int(floatCover)) + "x</b> (min 40x)</td></tr>"
					print(coverage)
				if(re.search(r'(Median)Insert', str(sheet.cell(row=wsRow-1 , column=ii).value), flags=re.IGNORECASE)):
				...
				if(re.search(r'total', str(sheet.cell(row=wsRow-1, column=ii).value), flags=re.IGNORECASE)):
					...
					genomeLen = "<tr><td><b>" + str(floatGenomeLen) + "MB</b> (5.5MB, 5% error margin)</td></tr>"
					#print(genomeLen)
		elif(re.search('CDC03-98', str(sheet.cell(row=wsRow , column=1).value) ) or re.search('CDC03-98', str(sheet.cell(row=wsRow ,column=2).value) ) ):
