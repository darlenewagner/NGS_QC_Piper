...
logger = logging.getLogger("salmWGSqc_openpyxl_1.py")
		...
		if(sheet.cell(row=wsRow, column=1).value == 'R1'):
			...
			for ii in range(1, 12):
				...
				if(re.search('depth', str(sheet.cell(row=wsRow-1 , column=ii).value), flags=re.IGNORECASE) ):
					...
					coverage = "<tr><td><b>" + str(int(floatCover)) + "x</b> (min 30x)</td></tr>"
					print(coverage)
				...
				if(re.search(r'total', str(sheet.cell(row=wsRow-1, column=ii).value), flags=re.IGNORECASE)):
					...
					genomeLen = "<tr><td><b>" + str(floatGenomeLen) + "MB</b> (5.0MB, 5% error margin)</td></tr>"
		elif(re.search('C6472-M3', str(sheet.cell(row=wsRow , column=1).value)) or re.search('C6472-M3', str(sheet.cell(row=wsRow ,column=2).value))):
